...
class AutoresponderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'reply_when' => ['required', 'in:all,group,personal'],
            'status' => ['required', 'in:active,inactive'],
            'type_keyword' => ['required', 'in:equal,contains'],
        ]);
        $table = new AutoResponder;

        $table->user_id = auth()->user()->id;
        $table->session_id = session()->get('main_device');
        $table->keyword = trim((string) $request->keyword);
    ...
    public function status(Request $request)
    {
        ...
        $table = AutoResponder::where('id', $request->id)->where('user_id', auth()->id())->where('session_id', session('main_device'))->firstOrFail();
    ...
    {
        $request->validate(['id' => ['required','array'], 'id.*' => ['integer']]);
        AutoResponder::whereIn('id', $request->id)->where('user_id', auth()->id())->where('session_id', session('main_device'))->delete();
