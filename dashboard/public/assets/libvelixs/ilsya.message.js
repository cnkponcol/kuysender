...
$(document).on('click', '.getfiles-ilsya', function () {
    ...
    $("input[name='media_storage_path']").val($(this).data('path') || '')
    const selectedMime = String($(this).data('mime') || '');
    if (selectedMime === 'image') $("select[name='media_type']").val('image');
    else $("select[name='media_type']").val('document');
...
$("select[name='message_type']").on('change', function () {
    ...
    switch (value) {
        case "text":
            ...
            break;
        case "media":
            html += '<div class="col-12 col-xl-6 col-lg-6"><div class="mb-3"><label class="form-label">Media</label><div class="input-group"><input type="text" class="form-control" name="media"><input type="hidden" name="media_storage_path"><button class="btn btn-primary waves-effect filemanagerilsya" type="button">Pilih File</button></div><div class="form-text">Gambar/PDF/dokumen dari File Manager atau URL HTTPS.</div></div></div>';
            html += '<div class="col-12 col-xl-6 col-lg-6"><div class="mb-3"><label class="form-label">Media Mime</label><select name="media_type" required class="form-select"><option value="">-- Select One --</option><option value="image">Image</option><option value="video">Video</option><option value="audio">Audio</option><option value="document">Document</option></select></div></div>';
